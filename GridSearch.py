...
######################## RUNNING THE TSETLIN MACHINE #################################
print("Initializing variables and starting TM...")


# wandb.init(project="IoTSecurity-GridSearch")

def train_tm(config):
    tm = TMClassifier(config["clauses"], config["T"], config["s"], config["Max_litterals"], platform='CUDA',
    ...
    return {"mean_accuracy": result, "epoch": config["epochs"]}


analysis = tune.run(
    train_tm,
    config={
        "clauses": tune.grid_search([1000, 2000, 3000]),
        "T": tune.grid_search([10, 15, 20]),
        "s": tune.grid_search([2, 3, 4]),
        "Max_litterals": tune.grid_search([18, 24, 32]),

...
class KDD99(TMUDataset):
    def __init__(self, split=0.7, shuffle=False, booleanize = True, max_bits_per_literal = 32, balance = True, class_size_cutoff = 5000):
        self.split = split
        ...
        if self.booleanize:
            ...
            for item in data_values:  # for each dataset item
                ...
                for feature in values:  # for each value / feature in said item_
                    ...
                    import struct
                    rowie += str(''.join('{:0>8b}'.format(c) for c in struct.pack('!f', kek)))[-self.max_bits_per_literal:]  # concatenate the binary string for each feature to the string representing the item
...
class NSLKDD(TMUDataset):
    def __init__(self, shuffle = True, booleanize = True, balance_train_set = True, balance_test_set = True, max_bits_per_literal = 16, class_size_cutoff = 500, limit_to_classes_in_train_set = True, limit_to_classes_in_test_set = True):
        self.shuffle = shuffle
        self.booleanize = booleanize
...
class UNSW_NB15(TMUDataset):
    ...
    def retrieve_dataset(self, paths) -> Dict[str, np.ndarray]:
        ...
        for path in paths:
            ...
            all_data_dict, class_registry = self.iot_data_to_binary_list(path, maximum_bits, all_data_dict, class_registry)
        print("Data distribution:", class_registry)
        # Count the size of each category above the threshold and find the size of the smallest one
        counts = []
        for key in class_registry.keys():
            print(key)
            ...
            for index, item in enumerate(data_values):  # for each dataset item
                ...
                for feature in values:  # for each value / feature in said item_
                    try:
                        ...
                        kek = float(non_floatable.index(feature))
                    import struct
                    rowie += str(''.join('{:0>8b}'.format(c) for c in struct.pack('!f', kek)))[-self.bits_per_entry:]  # concatenate the binary string for each feature to the string representing the item
                db[label].append([*rowie])
                registry[label] += 1
            return (db, registry)  # returns tuple of binary representation of data item and its label as an integer, and the string of labels for later decoding.
        else:

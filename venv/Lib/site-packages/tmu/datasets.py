...
class NSLKDD(TMUDataset):
    ...
    def retrieve_dataset(self, path_to_data_directory) -> Dict[str, np.ndarray]:
        from sklearn.datasets import fetch_openml
        import math
        import arff
        ...
        data = []
        labels = []
        file = arff.load(path_to_data_directory+"/KDDTrain+.arff")
        for row in file:
            row = list(row)
            data.append((row[:-1],row[-1]))
            labels.append(row[-1])
        file = arff.load(path_to_data_directory+"/KDDTest+.arff")
        for row in file:
            row = list(row)
            data.append((row[:-1],row[-1]))
            labels.append(row[-1])
